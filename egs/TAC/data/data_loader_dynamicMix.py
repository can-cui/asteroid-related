...
from data.dynamic_mixing import get1mix, get2mix, get3mix,get_S_spk_mix,get_S_spk_mic,get_reverb_mix_src,get_reverb_mix_src_dereverb
import logging
...
class SpectrogramParser(AudioParser):
    ...
    # def parse_speaker_list(self, speaker_list,dvector,dvector_dict):
    ...
    #         # spk_list = []
    #         for audio in spk:
    #             audio_path = os.path.join(constant.args.data_path, audio)
    #             wav_tensor, sample_rate = torchaudio.load(audio_path)
    #             mel = self.w2m(wav_tensor=wav_tensor,sample_rate=sample_rate)
    #             spk_mel_tensor.append(mel)
    #             # spk_dvect = dvector_dict[audio]
    ...
    #         with torch.no_grad():
    #             spk_dvect = dvector.embed_utterances(spk_mel_tensor)  # shape: (emb_dim)
    ...
    #         # spk_mean = spk_mean.unsqueeze(0)
    #         # spk_dvect = spk_dvect.unsqueeze(0)
    #         # M = 1
    #         # N = 1
    #         # A_dots = (spk_mean*spk_mean).sum(axis=1).reshape((M,1))*np.ones(shape=(1,N))
    #         # B_dots = (spk_dvect*spk_dvect).sum(axis=1)*np.ones(shape=(M,1))
    #         # D_squared =  torch.cdist(spk_mean.unsqueeze(0), spk_dvect.unsqueeze(0), p=2)
    #         # plt.imshow(D_squared)
    #         # # plt.show()  
    #         # plt.savefig('img/spk%s.png'%(spk[0].split('/')[1]))
    #         # plt.imshow(spk_dvect)
    #         # plt.show()  
    ...
    #         spk_embed = []
    #         spk_path = [os.path.join(constant.args.data_path, audio) for audio in spk]
...
class SpectrogramDataset(Dataset, SpectrogramParser):
    def __init__(self,
        ...
        # ids = torch.load(manifest_filepath_list)
        ...
        # # self.dvector_dict = torch.load(constant.args.dvector_path_pt)
        ...
        # with torch.no_grad():
        #     self.pad_dvector = self.dvectorInterface.directory_embed_utterance(torch.zeros(1000,80))
        # # self.all_spk_profil = (self.dvectorInterface,self.spk_dict,self.spk_dict_key)
        ...
        # if num_spk == 2:
        #     rir_path="/srv/storage/talc2@talc-data2.nancy.grid5000.fr/multispeech/calcul/users/ccui/datasets/LibriSpeech/rir_10.pt"
        # else:
        # rir_path="/srv/storage/talc2@talc-data2.nancy.grid5000.fr/multispeech/calcul/users/ccui/datasets/LibriSpeech/grid_1000rir_123_234_nbimg111_sameRt60.pt"
        # rir_path="/gpfswork/rech/rte/udh25oz/datasets/LibriSpeech/grid_1000rir_123_234_nbimg111_sameRt60.pt"
        # if num_spk == 1:
        #     # rir_path="/gpfswork/rech/rte/udh25oz/datasets/LibriSpeech/rir_spk1_234_1000.pt"
        #     # rir_path="/gpfswork/rech/rte/udh25oz/datasets/LibriSpeech/grid_10rir_1_2_nbimg111.pt"
        #     rir_path="/srv/storage/talc2@talc-data2.nancy.grid5000.fr/multispeech/calcul/users/ccui/datasets/LibriSpeech/grid_10rir_1_2_nbimg111.pt"
        #     # rir_path="/srv/storage/talc2@talc-data2.nancy.grid5000.fr/multispeech/calcul/users/ccui/datasets/LibriSpeech/grid_100rir_1_2_nbimg111.pt"
        # elif num_spk == 2:
        #     # rir_path="/gpfswork/rech/rte/udh25oz/datasets/LibriSpeech/grid_100rir_2_2_001.pt"
        #     # rir_path="/gpfswork/rech/rte/udh25oz/datasets/LibriSpeech/rir_spk2_2348_1000.pt"
        #     # rir_path="/gpfswork/rech/rte/udh25oz/datasets/LibriSpeech/rir_10.pt"
        #     rir_path="/srv/storage/talc2@talc-data2.nancy.grid5000.fr/multispeech/calcul/users/ccui/datasets/LibriSpeech/grid_100rir_2_2_nbimg111.pt"

            
        # elif num_spk == 3:
        #     # rir_path="/gpfswork/rech/rte/udh25oz/datasets/LibriSpeech/rir_spk3_2_rt041_10.pt"
        #     rir_path="/srv/storage/talc2@talc-data2.nancy.grid5000.fr/multispeech/calcul/users/ccui/datasets/LibriSpeech/grid_10rir_3_2_nbimg111.pt"
        ...
        # self.data_num = len(os.listdir(os.path.join(self.data_path,self.data_type,'2spk','2mic')))
        # self.data_num = len(ids[2][max_mics])
        # print("self.data_num")
        # print(self.data_num)
    ...
    # def get_dvector(speaker_profile,dvectorInterface):
    #     spk_directory = []
    #     # print(list(dvector_dict.keys())[:10])
    #     for spk_wavs in speaker_profile:
    #         spk_dvect = dvectorInterface.embed_utterances(spk_wavs)
    #         spk_directory.append(spk_dvect)
    #     spk_directory=torch.stack(spk_directory,dim=0)
    #     return spk_directory
    ...
    def __getitem__(self, index):
        ...
        if self.num_spk ==1: 
            ...
            rir_list = self.rir[mix_type][nmic]
            spk_rir_mix,spk_rir_src = random.sample(rir_list, 1)[0]
            mixtures,sources, texts_list = get_reverb_mix_src(max_len, mix_type,sample,self.spk_dict,self.data_path,nmic,spk_rir_mix,spk_rir_src) # dynamic mixing (only work for 1spk)
    ...
    def get_mix_src_txt(self,sample_dir,mix_type,nmic):
        ...
        for mic in range(nmic):
            ...
            for spk in range(mix_type):
                spk_path = os.path.join(sample_dir,'spk'+str(spk+1)+'_mic'+str(mic+1)+'.wav')
...
def _collate_fn(batch):
    ...
    src = torch.zeros(len(batch), batch[0][1].size(0),batch[0][1].size(1), max_seq_len) # B x Mic x Spk x T
