...
def get_reverb_mix_src_2ch(max_len, audio, spk_dict, wavin_dir, nmic, spk_rir_mix, spk_rir_src):
    text_spk1 = audio["wrd"]
    nspk = random.sample(range(1, 5), 1)[0]  # 1- 5 spk get_reverb_mix_src_dereverb
    # wav=audio["wav"].split("/")
    # wav=os.path.join(wav[-4],wav[-3],wav[-2],wav[-1])
    # wav=wav[:-5]+".wav"
    wav = audio["wav"]
