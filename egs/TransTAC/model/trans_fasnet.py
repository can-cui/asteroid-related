...
from .transformer import SBTransformerBlock, Dual_Path_Model
from asteroid.masknn import norms
from asteroid.masknn.tac import TAC
from asteroid.dsp.spatial import xcorr


class TransFasNetTAC(BaseModel):
    ...
    def __init__(
        ...
        self.encoder = nn.Conv1d(1,
                                 self.enc_dim,
                                 self.context * 2 + self.window,
                                 bias=False)
        self.enc_LN = norms.get(norm_type)(self.enc_dim)

        # DPRNN here + TAC at each layer
        self.bottleneck = nn.Conv1d(self.filter_dim + self.enc_dim,
                                    self.feature_dim,
                                    1,
                                    bias=False)

        self.Trans_TAC = nn.ModuleList([])
        ...
        for i in range(self.n_layers):
            ...
            if self.use_tac:
                tmp.append(
                    TAC(self.feature_dim, tac_hidden_dim, norm_type=norm_type))
            self.Trans_TAC.append(tmp)

        # DPRNN output layers
        self.conv_2D = nn.Sequential(
            nn.PReLU(),
            nn.Conv2d(self.feature_dim, self.n_src * self.feature_dim, 1))
    ...
    def windowing_with_context(x, window, context):
        ...
        unfolded = unfolded.reshape(batch_size, nmic, window + 2 * context,
                                    n_chunks)
        return (
    ...
    def forward(self, x, valid_mics=None):
        ...
        n_samples = x.size(
            -1)  # Original number of samples of multichannel audio
        all_seg, all_mic_context = self.windowing_with_context(
            x, self.window, self.context)
        ...
        enc_output = (self.encoder(
            all_mic_context.reshape(batch_size * n_mics * seq_length, 1,
                                    feats)).reshape(batch_size * n_mics,
                                                    seq_length,
                                                    self.enc_dim).transpose(
                                                        1, 2).contiguous()
                      )  # B*n_mics, seq_len, enc_dim
        enc_output = self.enc_LN(enc_output).reshape(batch_size, n_mics,
                                                     self.enc_dim,
                                                     seq_length)  # apply norm

        # For each context window cosine similarity is computed. The first channel is chosen as a reference
        ref_seg = all_seg[:, 0].reshape(batch_size * seq_length,
                                        self.window).unsqueeze(1)
        ...
        all_cos_sim = (all_cos_sim.reshape(batch_size, seq_length, n_mics,
                                           self.context * 2 + 1).permute(
                                               0, 2, 3, 1).contiguous())
        ...
        input_feature = self.bottleneck(
            input_feature.reshape(batch_size * n_mics, -1, seq_length))
        ...
        n_chunks = unfolded.size(-1)
        unfolded = unfolded.reshape(batch_size * n_mics, self.feature_dim,
        ...
        for i in range(self.n_layers):
            ...
            dprnn = self.Trans_TAC[i][0]
            unfolded = dprnn(unfolded)
            # print("--unfolded.shape after dpm--")
            # print(unfolded.shape)
            if self.use_tac:
                b, ch, chunk_size, n_chunks = unfolded.size()
                tac = self.Trans_TAC[i][1]
                unfolded = unfolded.reshape(-1, n_mics, ch, chunk_size,
                                            n_chunks)
                unfolded = tac(unfolded,
                               valid_mics).reshape(batch_size * n_mics,
                                                   self.feature_dim,
                                                   self.chunk_size, n_chunks)
        # Output, 2D conv to get different feats for each source
        unfolded = self.conv_2D(unfolded).reshape(
            batch_size * n_mics * self.n_src,
            self.feature_dim * self.chunk_size, n_chunks)
        ...
        all_mic_context = all_mic_context.unsqueeze(2).repeat(
            1, 1, self.n_src, 1, 1)
        ...
        all_bf_output = all_bf_output.view(batch_size, n_mics, self.n_src,
                                           seq_length, self.window)

        # Fold back to obtain signal
        all_bf_output = F.fold(
            all_bf_output.reshape(batch_size * n_mics * self.n_src, seq_length,
                                  self.window).transpose(1, -1),
            (n_samples, 1),
            kernel_size=(self.window, 1),
            padding=(self.window, 0),
            stride=(self.window // 2, 1),
        )
        bf_signal = all_bf_output.reshape(batch_size, n_mics, self.n_src,
                                          n_samples)
        ...
        if valid_mics.max() == 0:
            ...
            bf_signal = [
                bf_signal[b, :valid_mics[b]].mean(0).unsqueeze(0)
