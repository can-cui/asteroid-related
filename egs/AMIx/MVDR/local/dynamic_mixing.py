...
def get_mix_src_ami(spk_dict, nspk, nmic, sample):
        ...
        if len(filtered_spk) >= (nspk - 1):
            ...
            for other_spk in other_spks:
                ...
                for i in range(nmic):
                    ...
                    mixtures[i] += mixture
                    sources[i] += spk
    mixtures = torch.cat(mixtures, 0)  # torch.Size([2, 64000])
    # print(mixtures.shape)  # torch.Size([2, 64000])
    # # WPE
    # mixtures = torch.stack(mixtures, dim=1)  # torch.Size([1, 2, 64000])
    ...
    # print(sources.shape)
    # # WPE
    # Y_ = stft(sources, **stft_options)
    # # print(Y.shape)
    # Y_ = Y_.transpose(2, 0, 1)
    # Z_ = wpe(Y_)
    # sources = istft(
    #     Z_.transpose(1, 2, 0),
    #     size=stft_options["size"],
