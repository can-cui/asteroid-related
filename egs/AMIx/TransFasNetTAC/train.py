...
class TACSystem(System):
    def common_step(self, batch, batch_nb, train=True):
        ...
        # print("*" * 100)
        # print(batch)
        # tensor([3, 2], device='cuda:1')]
        # print(inputs.shape)
        # torch.Size([2, 6, 64000])
        # print(targets.shape)
        # torch.Size([2, 6, 2, 64000])
        ...
        loss = self.loss_func(
            est_targets, targets[:, 0]).mean()  # first channel is used as ref
...
def main(conf):
    ...
    train_set = AMIxDataset(conf["data"]["train_json"],
                            conf["data"]["segment"],
                            max_mics=conf["main_args"]["max_mics"],
                            train=True)
    val_set = AMIxDataset(conf["data"]["dev_json"],
                          conf["data"]["segment"],
                          max_mics=conf["main_args"]["max_mics"],
                          train=False)
    ...
    try:
        ...
        model = TransFasNetTAC(**conf["net"], sample_rate=conf["data"]["sample_rate"])
