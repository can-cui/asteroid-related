...
def main(job_nb):
    ...
    out_dir = "/srv/storage/talc2@talc-data2.nancy.grid5000.fr/multispeech/calcul/users/ccui/datasets/AMI/clips_synthesis_8array1_align_3spk"
    ...
    spk_comb = list(itertools.combinations(spk_list, 3))
    sample = 1
    for couple in spk_comb:
        ...
        spk3_samples = glob.glob(os.path.join(spk3, "*"))
        # Combine samples in pairs
        couple_com_list = list(itertools.product(spk1_samples, spk2_samples,spk3_samples))
        for couple_com in couple_com_list:
            ...
            arrays_spk3 = glob.glob(os.path.join(couple_com[2], "*"))
            mic_num = 1
